...
func TestManagedCluster(t *testing.T) {
	...
	cases := []struct {
		...
		/*{
			name:        "match with score quantities int",
			expr:        `managedCluster.scores("test-score").filter(s, s.name == 'cpu').all(e, quantity(e.quantity).isGreaterThan(quantity("2")))`,
			cluster:     testinghelpers.NewManagedCluster("test").Build(),
			expectValue: trueVal,
		},
		{
			name:        "not match with score quantities int",
			expr:        `managedCluster.scores("test-score").filter(s, s.name == 'cpu').all(e, quantity(e.quantity).isLessThan(quantity("2")))`,
			cluster:     testinghelpers.NewManagedCluster("test").Build(),
			expectValue: falseVal,
		},
		{
			name:        "match with score quantities string",
			expr:        `managedCluster.scores("test-score").filter(s, s.name == 'memory').all(e, quantity(e.quantity).isGreaterThan(quantity("200Mi")))`,
			cluster:     testinghelpers.NewManagedCluster("test").Build(),
			expectValue: trueVal,
		},
		{
			name:        "not match with score quantities string",
			expr:        `managedCluster.scores("test-score").filter(s, s.name == 'memory').all(e, quantity(e.quantity).isLessThan(quantity("200Mi")))`,
			cluster:     testinghelpers.NewManagedCluster("test").Build(),
			expectValue: falseVal,
		},*/
		{
