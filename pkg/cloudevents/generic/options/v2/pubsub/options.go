...
type PubSubOptions struct {
	...
	// Insecure indicates whether to use an insecure connection (without TLS).
...
type PubSubConfig struct {
	...
	// (Optional) Insecure indicates whether to use an insecure connection (without TLS).
...
func BuildPubSubOptionsFromFlags(configPath string) (*PubSubOptions, error) {
	...
	if config.Insecure && config.Endpoint == "" {
		return nil, fmt.Errorf("endpoint must be specified when insecure is true")
	}
