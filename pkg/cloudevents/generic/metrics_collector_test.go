...
func TestCloudEventsMetrics(t *testing.T) {
	...
	for _, c := range cases {
		...
		t.Run(c.name, func(t *testing.T) {
			...
			agentOptions := fake.NewAgentOptions(sendReceiver, c.clusterName, testAgentName)
			...
			// wait 1 seconds for agent receive the resources
			time.Sleep(time.Second)

			// mimic agent disconnection
			agent.resetClient(nil)
			...
			require.Equal(t, 0.0, toFloat64Counter(reconnectTotal))
