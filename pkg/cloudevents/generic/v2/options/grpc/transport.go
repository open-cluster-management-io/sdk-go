...
func ToProtobufEvent(evt cloudevents.Event) (*pbv1.CloudEvent, error) {
	...
	pbEvt.Attributes[options.Prefix+options.TimeAttrKey] = timeAttr
	...
	for key, val := range evt.Extensions() {
		...
		pbEvt.Attributes[options.Prefix+key] = attr
	}
...
func ToCloudEvent(pbEvent *pbv1.CloudEvent) (cloudevents.Event, error) {
	...
	for key, val := range pbEvent.Attributes {
		...
		evt.SetExtension(strings.TrimPrefix(key, options.Prefix), extension)
	}

	if err := evt.SetData(cloudevents.ApplicationJSON, pbEvent.GetBinaryData()); err != nil {
		return evt, err
	}
	return evt, nil
}
