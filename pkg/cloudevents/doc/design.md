...
**Note**: It is recommended to use the `ManifestWorkSourceClient` with `SourceInformerWatcherStore` building a `ManifestWorkInformer`. If using a `ManifestWorkSourceClient` with `SourceLocalWatcherStore` to build an informer, there will be two caches for manifestworks, it will increase memory usage.
