...
func TestManagedClusterAddOnCodec_EventDataType(t *testing.T) {
	...
	if dataType.Group != addonapiv1beta1.GroupVersion.Group {
		t.Errorf("expected Group %s, got %s", addonapiv1beta1.GroupVersion.Group, dataType.Group)
	}
	if dataType.Version != addonapiv1beta1.GroupVersion.Version {
		t.Errorf("expected Version %s, got %s", addonapiv1beta1.GroupVersion.Version, dataType.Version)
	}
...
func TestManagedClusterAddOnCodec_Decode(t *testing.T) {
	...
	if len(decoded.Spec.Configs) != 1 {
		t.Errorf("expected 1 config, got %d", len(decoded.Spec.Configs))
	}
	if len(decoded.Spec.Configs) > 0 && decoded.Spec.Configs[0].ConfigReferent.Name != "test-config" {
		t.Errorf("expected config name test-config, got %s", decoded.Spec.Configs[0].ConfigReferent.Name)
	}
}
