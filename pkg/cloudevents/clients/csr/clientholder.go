...
import (
	...
	"open-cluster-management.io/sdk-go/pkg/cloudevents/clients/csr/agent"
...
func (h *ClientHolder) Clients() *agent.CSRClient {
	return h.client
}
...
func NewAgentClientHolder(ctx context.Context, opt *options.GenericClientOptions[*certificatev1.CertificateSigningRequest]) (*ClientHolder, error) {
	...
	csrClient := agent.NewCSRAgentClient(cloudEventsClient, opt.WatcherStore(), opt.ClusterName())
